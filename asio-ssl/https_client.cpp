...
private:
  ...
  void on_handshake(const error_code& error) {
    ...
    asio::async_write(socket_, request_, BIND_2(on_send));
  }
    ...
    switch(type) {
      case receive_type::status_code: {
        asio::async_read_until(socket_, response_, "\r\n",
          std::bind(&https_client::on_receive, this, std::placeholders::_1, receive_type::status_code));
        break;
      }
      case receive_type::header: {
        asio::async_read_until(socket_, response_, "\r\n\r\n",
          std::bind(&https_client::on_receive, this, std::placeholders::_1, receive_type::header));
        break;
      }
      case receive_type::body: {
        asio::async_read(socket_, response_, asio::transfer_at_least(1),
          std::bind(&https_client::on_receive, this, std::placeholders::_1, receive_type::body));
        break;
      }
      default: {
        break;
